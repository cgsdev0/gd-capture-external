...
Texture2DRDAutoRelease::Texture2DRDAutoRelease(
    RenderingDevice::DataFormat format, int cx, int cy,
...
void CaptureBase::_bind_methods() {
  ...
  ADD_SIGNAL(
      MethodInfo("texture_changed",
                 PropertyInfo(Variant::OBJECT, "texture", PROPERTY_HINT_NONE,
                              "", PROPERTY_USAGE_DEFAULT, "Texture2D")));
}

void CaptureBase::create_texture(RenderingDevice::DataFormat format, int cx,
                                 int cy, bool invert_rb) {
  ...
  texture =
      Ref(memnew(Texture2DRDAutoRelease(format, cx, cy, bytes, invert_rb)));
...
Ref<Texture2DRD> CaptureBase::get_texture() const { return texture; }
